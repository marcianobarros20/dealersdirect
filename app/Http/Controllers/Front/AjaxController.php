...
class AjaxController extends Controller
{
    ...
    public function GetAllRequest(){
        ...
        foreach ($RS as $key => $value) {
            ...
            if($countimg!=0){
                $imx=EdmundsMakeModelYearImage::where('make_id',$value->requestqueue->make_id)->where('model_id',$value->requestqueue->carmodel_id)->where('year_id',$value->requestqueue->year)->groupBy('title','edmunds_path_small')->get();
             $RS[$key]['imx']=$imx;
            }else{
               $RS[$key]['imx']=""; 
            }
